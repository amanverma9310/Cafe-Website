...
export default function AdminLayout() {
  ...
  if (status === "loading") {
    return (
      <div className="admin-ui min-h-screen bg-stone-50">
  ...
  const sidebar = (
    <nav
      ...
      className="flex h-full flex-col overflow-y-auto px-3 py-5"
    >
      {/* Logo / dashboard link */}

      <Link
        to="/admin/dashboard"
        className="mb-6 flex items-center gap-2 px-3 text-lg font-bold text-emerald-900"
      >
        <span className="grid size-8 place-items-center rounded-lg bg-emerald-800 text-white">
          A
        </span>

        <span>Agama admin</span>
      </Link>
          ...
          if (item.group) {
            return (
              <li
                key={item.group}
                className={cn(
                  "px-3 pb-1 text-xs font-semibold uppercase tracking-wide text-stone-400",
                  index > 0 && "pt-5",
                )}
              >
                {item.group}
          ...
          return (
            <li key={item.to}>
              <NavLink
                /*
                 * IMPORTANT:
                 * Use the full admin URL instead of a relative URL.
                 *
                 * Example:
                 * menu -> /admin/menu
              ...
              >
                ...
                {item.badge && unreadCount > 0 ? (
                  <span
                    ...
                    aria-label={`${unreadCount} unread enquiries`}
                  >
                    {unreadCount}
                  </span>
                ) : null}
  ...
  return (
    <div className="admin-ui min-h-screen bg-stone-50 text-stone-900">
      <Helmet>
        <title>Admin | Agama Cafe & Bar</title>
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-20 flex h-14 items-center justify-between border-b border-stone-200 bg-white/90 px-4 backdrop-blur sm:px-8">
          ...
          <button
            type="button"
            className="grid size-10 place-items-center rounded-lg hover:bg-stone-100 lg:hidden"
          ...
          <div className="flex items-center gap-1">
            <a
              ...
              className="inline-flex min-h-9 items-center gap-1.5 rounded-lg px-3 text-sm font-medium text-stone-700 hover:bg-stone-100"
            >
              View site
